...
void activateNode(Network *nn, LayerType ltype, int id, bool train){
    ...
    if (actFct==TANH)   {
        ...
        n->output_diff_arith = test_type(tanh(double(n->output_diff_arith)));
    }
    else if (actFct==SIGMOID) {
        // if (!train) {
        //     std::cout << "before sigmoid:"  << n->output <<  " " << double(n->output_diff_arith) << std::endl;
        // }
        // std::cout <<  double(n->output_diff_arith) << std::endl;
        n->output= 1 / (1 + (exp((double)-n->output)) );
        n->output_diff_arith = 1 / (1 + (exp((double)-n->output_diff_arith)) );
        // if (!train) {
        //     std::cout << "after sigmoid:" << n->output <<  " " << double(n->output_diff_arith) << std::endl;
        // }
    }
    else if (actFct==RELU) {
        n->output = (n->output <= 0)? 0 : n->output;
        n->output_diff_arith = (double(n->output_diff_arith) <= 0)? test_type(0) : n->output_diff_arith;
    }
}
...
void calcNodeOutput(Network *nn, LayerType ltype, int id, bool train){
    ...
    calcNode->output_diff_arith = calcNode->bias_diff_arith;

    
    for (int i=0; i<prevLayer->ncount;i++){
        Node *prevLayerNode = (Node*)sbptr;
        // if (i == 0 && ltype==OUTPUT) {
        //     std::cout << "prevLayer : " << prevLayerNode->output << std::endl; 
        //     std::cout << "prevLayer diff arith : " << double(prevLayerNode->output_diff_arith)  << std::endl; 
        // }
        calcNode->output += prevLayerNode->output * calcNode->weights[i];
        calcNode->output_diff_arith += (prevLayerNode->output_diff_arith * calcNode->weights_diff_arith[i]);
        // if ( !train && ltype==HIDDEN) {
        //     std::cout << "double: prev act, weight, prev act * weight, ACC : " << prevLayerNode->output << " " <<  calcNode->weights[i] << " " << prevLayerNode->output * calcNode->weights[i]<< " " << calcNode->output  << std::endl; 
        //     std::cout << "arith:  prev act, weight, prev act * weight, ACC : " << double(prevLayerNode->output_diff_arith) << " " <<  double(calcNode->weights_diff_arith[i]) << " " << double(prevLayerNode->output_diff_arith * calcNode->weights_diff_arith[i]) << " " << double(calcNode->output_diff_arith)  << std::endl; 
        //     //std::cout << "diff arith: act, weight : " << double(calcNode->output_diff_arith) << " " <<  double(calcNode->weights_diff_arith[i]) << std::endl; 
        // }
        sbptr += prevLayerNodeSize;
    }
    // std::cout << "double:     act, weight : " << calcNode->output << std::endl; 
    // std::cout << "diff arith: act, weight : " << double(calcNode->output_diff_arith)  << std::endl; 
...
void calcLayer(Network *nn, LayerType ltype, bool train){
    Layer *l;
    l = getLayer(nn, ltype);
    
    for (int i=0;i<l->ncount;i++){
        calcNodeOutput(nn, ltype, i, train);
        activateNode(nn,ltype,i, train);
    }
}
...
void feedForwardNetwork(Network *nn, bool train){
    calcLayer(nn, HIDDEN, train);
    calcLayer(nn, OUTPUT, train);
}
...
void feedInput(Network *nn, Vector *v) {
    ...
    for (int i=0; i<v->size;i++){
        iln->output = v->vals[i];
        iln->output_diff_arith = v->vals_diff_arith[i];
...
Layer *createInputLayer(int inpCount){
    ...
    Layer *il = (Layer*)malloc(inpLayerSize);
    il->ncount = inpCount;
    
    // Create a detault input layer node
    Node iln;
    iln.bias = 0;
    iln.bias_diff_arith = 0;
    iln.output = 0;
    iln.output_diff_arith = 0;
    iln.wcount = 0;
...
Layer *createLayer(int nodeCount, int weightCount){
    ...
    dn->bias = 0;
    dn->bias_diff_arith = 0;
    dn->output = 0;
    dn->output_diff_arith = 0;
    dn->wcount = weightCount;
    for (int o=0;o<weightCount;o++){
        dn->weights[o] = 0; // will be initialized later
        dn->weights_diff_arith[o] = 0; // will be initialized later
    }
...
void promoteWeightsLayer(Network *nn, LayerType ltype) {
    ...
    for (int o=0; o<l->ncount;o++){
        ...
        // copy / transform each weight
        for (int i=0; i<n->wcount; i++){
            n->weights_diff_arith[i] = n->weights[i];
            // std::cout << n->weights[i] << " " << float(n->weights_diff_arith[i]) << " " << double(n->weights_diff_arith[i]) << std::endl;
        }
        
        // copy / transform bias
        n->bias_diff_arith = n->bias;
...
void promoteWeights(Network *nn) {
    promoteWeightsLayer(nn, HIDDEN);
    promoteWeightsLayer(nn, OUTPUT);
...
/**
 * @brief Returns the network's classification using the ID of the node with the hightest output
 * @param nn A pointer to the NN
 */
...
int getNetworkClassification(Network *nn, int * classifciation_diff_arith){
    ...
    double maxOut = 0;
    double maxOut_diff_arith = 0;
    int maxInd = 0;
    int maxInd_diff_arith = 0;
    ...
    for (int i=0; i<l->ncount; i++){
        ...
        // std::cout << on->output << " " << double(on->output_diff_arith) << std::endl; 
        if (double(on->output_diff_arith) > maxOut_diff_arith){
            maxOut_diff_arith = double(on->output_diff_arith);
            maxInd_diff_arith = i;
        }
    }
  
    *classifciation_diff_arith = maxInd_diff_arith;
    return maxInd;
}
        ...
        if (layer->ncount>0){
            ...
            for (int n=0; n<10; n++){
                ...
                for (int c=0; c<connCount; c++){
                    ...
                    float w_diff_arith = node->weights_diff_arith[c]; 
                    if (c<topLast || c>=connCount-topLast) printf("%5d:%9f:%9f",c,w,w_diff_arith);
                }
                printf("\n");
